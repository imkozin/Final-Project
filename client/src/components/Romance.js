    ...
    const getRomance = async () => {
        try {
            ...
            if (res.data.length > 0) {
                const genre = res.data.filter(book => book.genres.includes('Romance'));
                const sortedData = genre.sort((a, b) => b.rating_count - a.rating_count).slice(0, 10);
                console.log(sortedData);
                setRomance(sortedData);
            }
        } catch (err) {
